...
def train_vae(model, train_loader, optimizer, device,epoch):
    ...
    for batch_idx, data in enumerate(TrainLoader):
        ...
        loss1 = vae_loss(recon_x, x, mu[:,8:], logvar[:,8:])
        mu_label_loss = F.mse_loss(mu[:,0:2], use1[:,0:2])
        mu_label_loss2 = F.mse_loss(mu[:,4:8], use1[:,4:8])

        loss = loss1 +  mu_label_loss+mu_label_loss2
        loss.backward()
        optimizer.step()
...
if __name__ == '__main__':
    ...
    # Save model
        torch.save(vae_model.state_dict(), 'vae_partial.pth')
