...
class L2P_better_Decoder(nn.Module):
    def __init__(self, state_size=4, m_channel=16, output_channel=3):
        ...
        self.conv1 = nn.ConvTranspose2d(m_channel, m_channel, kernel_size=4, stride=2, padding=1)
    ...
    def forward(self, states):
        ...
        outputs = []
        for i in range(3):
            out = F.relu(self.conv1(x[:, i]))
            out = self.conv[i](out)
            outputs.append(out.unsqueeze(1))
        x = torch.cat(outputs, dim=1)
        x = torch.sigmoid(x)
        img = torch.clamp(x.sum(dim=1) - 2, max=1.0,min=0.0)
        return x, img
    ...
    def train_model(self, images_pole, states, device, epochs=10, batch_size=64, learning_rate=0.001, val_split=0.1, lambda_size=0.0):
        ...
        for epoch in range(epochs):
            ...
            for batch_images, batch_states in train_loader:
                ...
                loss = (1-lambda_size)*criterion(outputs, real_images[:, 1:, ::]) + lambda_size * criterion(img, real_images[:, 0, ::].view(-1, 3, 96, 96))
                loss.backward()
                optimizer.step()
        ...
        with torch.no_grad():
            for batch_images, batch_states in val_loader:
                ...
                loss = criterion(img, real_images)
